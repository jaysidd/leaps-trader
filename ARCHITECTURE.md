...
- **Redis Socket Timeouts**: 5s connect + 10s read/write timeouts (`cache.py`) — prevents app hang on Redis unavailability
...
### 2026-02-14 — PresetSelector 98% Coverage: 28 Additional Tests
- **Enhanced**: `tests/pipeline/test_preset_selector_comprehensive.py` — Added 28 new tests across 5 new test classes: TestSelectPresetsE2E (5 async E2E tests for `select_presets()`), TestValidatePresetCatalog (6 tests for startup validation), TestGetPresetSelectorSingleton (3 tests for singleton pattern), TestGatherSnapshotEdgeCases (8 async tests for partial/empty dict handling), TestBuildReasoningEdgeCases (6 tests for F&G labels and missing signals).
- **Test count**: 113 tests (112 pass, 1 skip) in comprehensive file. 222 pipeline total. Closes previously untested code paths: `select_presets()` (0%→100%), `_validate_preset_catalog()` (0%→100%), `get_preset_selector()` singleton (0%→100%).

### 2026-02-14 — Replay → PresetSelector E2E Integration Tests
- **Fixed (P1)**: `scripts/replay/replay_services.py` — Confidence scale bug in `_compute_regime_from_bars()`. Both live MarketRegimeDetector and replay returned confidence on 1-10 scale, but PresetSelector expects 0-100. Added `confidence = min(int(confidence * 12.5), 100)` normalization. Scale: 5→63, 6→75, 7→88, 8→100. Live detector bug logged as P1 in BACKLOG.md.
- **New**: `tests/replay/__init__.py` — Package init for replay test directory.
- **New**: `tests/replay/spy_fixtures.py` — 5 deterministic synthetic SPY DataFrame generators (seeded RNG) + parameterized mid-band discovery: `make_bull_spy_bars()` (400→530 uptrend, VIX~8), `make_neutral_spy_bars()` (~480 oscillation, VIX~13), `make_bear_spy_bars()` (520→440 downtrend, VIX~25), `make_panic_spy_bars()` (500 flat then crash to 380, VIX~40), `make_mild_bull_spy_bars()` (470→510 gentle uptrend, VIX~12), `make_mid_band_spy_bars()` (parameterized), `MID_BAND_CANDIDATES` (7 discovery combos).
- **New**: `tests/replay/conftest.py` — Shared fixtures: ReplayClock (2026-02-10 10:00 ET), mock DB, autouse singleton cache cleanup (regime detector + preset selector), PRESET_CATALOG_STRICT=false.
...
### 2026-02-15 — Staging Redis Crash-Loop Fix + Telegram Isolation
- **Fixed**: Staging Redis crash-loop caused by corrupted volume config. Railway `--duplicate` creates a new Redis volume but the original Redis had written a `redis.conf` to it — the new instance inherited a corrupted config. Fix: staging Redis start command now cleans `*.conf` files before starting.
- **Fixed**: `services/cache.py` — Increased Redis `socket_timeout` from 5s to 10s to reduce transient timeout errors during staging Redis restarts.
- **Fixed**: Disabled Telegram bot in staging by clearing `TELEGRAM_BOT_TOKEN` — only one bot instance can poll `getUpdates` at a time, so staging and production cannot both run the bot.
...
- **Fixed**: `services/cache.py` — Added `socket_connect_timeout=5` and `socket_timeout=10` to both Redis client constructors. Previously had infinite timeouts which caused app hang when Redis was unavailable.
