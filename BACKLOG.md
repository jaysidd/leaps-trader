...
| ~~2~~ | ~~`user_alerts.alert_scope` column missing in DB~~ | ~~`models/user_alert.py`~~ | **FIXED 2026-02-13** — `scripts/fix_user_alerts_columns.py` adds 6 missing columns. |
...
| 7 | ~~No CI/CD pipeline~~ | — | **PARTIALLY ADDRESSED 2026-02-15** — Staging environment now auto-deploys from `main`, production from `prod`. Still no automated test runs on PR (GitHub Actions). |
...
| 22 | Migrate to typed StrategyProfile objects | `preset_selector.py`, `presets_catalog.py` | Replace string preset names with typed dataclasses. Eliminates typo bugs at import time, enables IDE autocompletion. Phase 2 of preset catalog extraction. |
...
- **Staging Environment + Infrastructure Hardening** (2026-02-15): Created `staging` Railway environment (cloned from production) with isolated DB, Redis, and domains. Created `prod` branch — production deploys from `prod`, staging from `main`. Fixed Redis infinite socket timeouts (added 5s connect+read timeouts in cache.py). Added circuit breaker to Redis log sink (skips writes 60s after failure). Changed Railway healthcheck from `/health` to `/` (Redis job status checks were timing out). Merged PR #3 main→prod. Files: cache.py, log_sink.py, railway.toml.
- **PresetSelector Showstopper Fix + Comprehensive Tests** (2026-02-14): Fixed 5 bugs (B1: `value_deep`→`deep_value` typo, B2: TTL mismatch 10→5min, B3: `confidence=0` falsy bug, B4: stale docstring, B5: silent fallback in automation). Extracted LEAPS_PRESETS into `app/data/presets_catalog.py` with `resolve_preset()` fail-fast helper + `get_catalog_hash()`. Added startup validation + version breadcrumbs (`selector_version`, `catalog_hash`). Created 86 comprehensive tests (85 pass, 1 skip) across 13 test classes + reusable test case doc. Files: presets_catalog.py (new), preset_selector.py, screener.py, main.py, replay_trading_day.py, test_preset_selector_comprehensive.py (new), PRESET_SELECTOR_TESTS.md (new).
- **Replay Harness: Full E2E Pipeline** (2026-02-13): Wired Screening Engine, AI Validator, Risk Gateway, and Position Sizer into replay harness — now tests all 7 pipeline layers end-to-end. Added `--skip-screening`, `--no-ai`, `--no-risk-check` CLI flags. Synthetic LEAPS options chain for screening soft-pass. Virtual BotConfig/BotState with realistic trading limits. AI validator gracefully falls back when Claude unavailable. Verified: 2 wins/$22.90 P/L on SSRM, 416 tests pass. Files: replay_trading_day.py, replay_services.py.
- **Fix Robinhood 2FA verification flow** (2026-02-13): robin_stocks' `_validate_sherrif_id()` calls `input()` for SMS/email codes, blocking FastAPI forever. Fixed by monkey-patching the function to raise `VerificationRequired` with challenge details, then submitting the code directly to Robinhood's `/challenge/{id}/respond/` API. New `/verify` endpoint, updated frontend modal to route MFA codes through `submitMFA` → verify endpoint. Files: robinhood_service.py, portfolio.py, Portfolio.jsx, portfolioStore.js, portfolio.js (API).
- **Historical Replay Harness** (2026-02-13): New `scripts/replay/` package replays past trading days through the real signal pipeline. Pre-fetches bars from Alpaca Historical API, advances simulated clock bar-by-bar, runs REAL signal engine code. Monkey-patches 7 singleton methods + datetime.now() in 2 modules. Verified: 5 signals, 3 trades, 2 wins/1 loss on 2026-02-10 test run. Zero production code changes, 414 tests pass. Files: scripts/replay/__init__.py, replay_services.py, replay_trading_day.py.
- **Replay PresetSelector + Audit Logging** (2026-02-13): Wired PresetSelector (market analysis → preset choice) into replay harness. `ReplayMarketIntelligence` computes regime/F&G/readiness from SPY daily bars + loads MRI from DB snapshots. New `ReplayAuditLog` model captures every pipeline decision (preset_selection, signal_generation, trade_execution, position_exit, summary) with JSON decision blobs. Verified market intelligence varies correctly across dates. 416 tests pass. Files: replay_services.py, replay_trading_day.py, replay_audit_log.py (new model).
- **Sentry removal + DB fix + pipeline diagnostic tooling** (2026-02-13): Removed never-activated Sentry SDK (no account existed). Fixed `user_alerts.alert_scope` missing column error (recurring every 5min). Created `scripts/diagnose_pipeline.py` — tests all 7 pipeline layers with real data, color-coded results, supports `--layer N`, `--execute`, `--api-guide` modes. Files: requirements.txt, main.py, config.py, scripts/fix_user_alerts_columns.py, scripts/diagnose_pipeline.py.
- **E2E pipeline test environment + quality gate fixes** (2026-02-13): Built comprehensive test suite (109 tests, 10 files) covering all 7 pipeline layers with controlled mock data. Fixed 5 weak thresholds that let marginal stocks (like SN: 85% signal engine, 3/10 AI batch) through: gate penalty cap -15→-25, MIN_COMPOSITE_SCORE 20→30, AI validator prompt rewrite (removed "be practical"), CONFIDENCE_THRESHOLD 65→70, MIN_CONFIDENCE 60→62. Tests run in ~1s with zero external API calls. Branch: test/e2e-pipeline. Files: signal_engine.py, engine.py, signal_validator.py, tests/pipeline/*.py.
...
- **Auto-scan defaults fix**: auto_scan_enabled was `false` by default with no UI toggle, so auto-scan never ran on Railway. Fixed via one-time seed migration + Autopilot UI controls. Added Redis log sink + /api/v1/logs endpoint + Logs page.
...
- **PresetSelector weighted scoring overhaul** (2026-02-13): Old priority-chain classification let `readiness_label="yellow"` (score 34-66, very common) veto bullish regime + low MRI, permanently locking scanner to `cautious` (conservative + blue_chip_leaps only). Replaced with weighted composite scoring: regime 35%, MRI 30%, F&G 20%, readiness 15% → -100/+100 score. Also fixed F&G CNN API fallback (non-200 now uses VIX-based calc instead of returning None). Result: `moderate_bull` with 3 presets instead of stuck at `cautious` with 2. Files: preset_selector.py, market_data.py, autopilot.py, main.py.
- **StrategySelector HIGH confidence fix** (2026-02-13): 0/486 stocks reached HIGH confidence (needed for auto-trading) despite 22 qualifying for 1d timeframe. Two issues: (1) "missing SMA data" from FMP 403 rate limits was wrongly classified as serious risk edge case — it's just data unavailability. (2) Single-timeframe HIGH threshold 68 too conservative for stocks that passed 4-stage screening. Fixed: removed "missing sma" from serious edges, lowered threshold to 65. File: strategy_selector.py.
